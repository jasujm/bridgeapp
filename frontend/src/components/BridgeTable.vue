...
function scoreMessage(
  { contract, tricksWon, result }: DealEndEvent,
  position: Position | null
) {
  if (contract && tricksWon && result.partnership) {
    const resultMessage = `Declarer made ${tricksWon} tricks.`;
    const contractMadeMessage = `Contract ${
      tricksWon >= contract.bid.level + 6 ? "made" : "defeated"
    }.`;
    let scoreMessage = "";
    if (position) {
      const who =
        partnershipFor(position) == result.partnership
          ? "You score"
          : "Opponent scores";
      scoreMessage = `${who} ${result.score} points`;
    } else {
      scoreMessage = `${partnershipText(result.partnership)} scores ${
        result.score
      } points`;
    }
...
export default class BridgeTable extends Vue {
  ...
  private fetchPlayerState() {
    if (this.gameId) {
      const api = this.$store.state.api;
      api
        .getPlayerState(this.gameId)
        .then((me: PlayerState) => (this.me = me))
        .catch((err: Error) => this.$store.dispatch("reportError", err));
    }
  }
  ...
  private fetchGameState() {
    if (this.gameId) {
      ...
      api
        .getGame(this.gameId)
        .then(({ game, counter }: GameCounterPair) => {
          this.name = game.name;
  ...
  private changePlayer({ position, player }: PlayerEvent) {
    if (player) {
      ...
      api
        .getPlayer(player)
        .then((p: Player) => (this.players[position] = p))
        .catch((err: Error) => this.$store.dispatch("reportError", err));
    } else {
  ...
  private cardPlayed({
    ...
    let index = _.findIndex(hand, (c) =>
      c ? c.rank == card.rank && c.suit == card.suit : false
    );
  ...
  private makeCall(call: Call) {
    if (this.gameId) {
      this.$store.state.api
        .makeCall(this.gameId, call)
        .catch(this.handleConflictError.bind(this));
    }
  }
  ...
  private playCard(card: Card) {
    if (this.gameId) {
      this.$store.state.api
        .playCard(this.gameId, card)
        .catch(this.handleConflictError.bind(this));
    }
  }

  private wrapEventHandler<Event extends AnyEvent>(
  ...
  private startGame() {
    this.close();
    this.ws = this.$store.state.api.subscribe(this.gameId, {
  ...
  private canReplaceDisplayTrick(trick: Trick | null) {
    ...
    return _.isEqual(
      trick.cards.slice(0, prefixLength),
      this.displayTrick.cards.slice(0, prefixLength)
    );
  }
