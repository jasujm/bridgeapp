...
describe("TrickDisplay.vue", function() {
    ...
    it("should display cards played in the trick", function() {
        const propsData = {
            trick: {
                cards: [
                    { position: Position.south, card: { rank: Rank._10, suit: Suit.hearts } },
                    { position: Position.west, card: { rank: Rank._7, suit: Suit.clubs } },
                ]
            }
        };
        const wrapper = mount(TrickDisplay, { localVue, propsData });
        expect(wrapper.find(".card-display.rank-10.suit-hearts").exists()).to.be.true;
