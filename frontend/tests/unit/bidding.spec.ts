...
function makeCalls(type: CallType, bid?: Bid) {
  return {
    calls: [makeCall(Position.north, type, bid)],
  };
}
...
describe("Bidding.vue", function () {
  ...
  const calls = [
    ...
    {
      propsData: makeCalls(CallType.redouble),
      expectedClass: ".type-redouble",
    },
  ];
  ...
  for (const { propsData, expectedClass } of calls) {
    it(`should display ${propsData.calls[0].call.type}`, function () {
      ...
      expect(wrapper.find(`.position-north .call${expectedClass}`).exists()).to
        .be.true;
    });
  }
