...
describe("api", function() {
    describe("getErrorMessage", function() {
        ...
        it("should return if there is no response", function() {
            ...
            expect(errorMessage).to.have.property("severity").that.is.equal(ErrorSeverity.danger);
        });
        ...
        for (const status of [404, 409]) {
            it(`should return warning on ${status}`, function() {
                ...
                const errorMessage = getErrorMessage(error);
                expect(errorMessage).to.have.property("severity").that.is.equal(ErrorSeverity.warning);
                expect(errorMessage).to.have.property("message").that.is.equal("warning");
            });
        }
    });
})
